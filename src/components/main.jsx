...
async function getData(page, setPersonajes, setMyError) {
  try {
    const response = await fetch(
      `https://rickandmortyapi.com/api/characterAAAA/?page=${page}`,
    );
    ...
    /*
			...
			*/
    if (!response.ok) {
      // uso la propiedad ok del objeto response, si response.ok=false, al negarlo, con el (!), se hace true y entra en el if statement
      throw new Error(
        `hubo un error en la consulta a la API! - Status: ${response.status}`,
      ); // asi se lanza un error
    }
    ...
    if (resData.results) {
      ...
      throw new Error(
        'No results found in API response (aqui pueden poner su mensaje)',
      ); // si resData.results no tiene info, tira un error. "el texto del error lo pueden escribir uds, es solo un string"
    }
  } catch (error) {
    // aqui maneja otro tipos de errores, no si la URL estuvo mal o no...
